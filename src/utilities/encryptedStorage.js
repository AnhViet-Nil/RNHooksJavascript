...
async function setToken(accessToken = '', refreshToken = '') {
	try {
		...
		throw new Error(`Cannot save encrypted memory! ${error}`);
	}
}
...
async function getToken() {
	try {
		...
		throw new Error(`Cannot access encrypted memory! ${error}`);
	}
}
...
async function removeToken() {
	try {
		...
		throw new Error(`Cannot remove encrypted memory! ${error}`);
	}
}
